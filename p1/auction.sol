...
function numberOfBidders() public view returns (uint256){
    return bidders.length;
}

function buyCar() public returns (bool){
    require(highestBidder == msg.sender, "You are not the winner of the auction");
    require(bids[msg.sender] == highestBid);
    require(now > auction_end ,"Auction is not over!");
